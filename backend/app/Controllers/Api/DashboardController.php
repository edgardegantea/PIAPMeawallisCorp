...
/**
 * GET /api/dashboard
 * Retorna un snapshot personal del usuario autenticado:
 *   - tasks_summary  : conteos y las 8 más urgentes
 *   - projects       : proyectos activos donde participo
 *   - time_this_week : horas registradas esta semana
 *   - alerts         : top-5 alertas (reutiliza lógica de NotificationsController)
 *   - activity       : últimas 10 entradas de actividad de mis proyectos
 */
class DashboardController extends BaseController
{
    public function index(): ResponseInterface
    {
        ...
        /* ── 1. Mis tareas (solo proyectos activos) ───────────────────────── */
        $allTasks = $db->query("
        ...
        $myProjects = $db->query("
            SELECT DISTINCT p.id, p.code, p.name, p.status, p.completion_percentage,
                   p.planned_end_date, c.name AS category_name, c.color AS category_color,
                   pm.role AS my_role
            FROM projects p
        ...
        ", [$weekStart, $userId])->getRow()->cnt ?? 0);

        /* ── 4. Top-5 alertas (igual que NotificationsController) ─────────── */
        $alerts = [];
        ...
        foreach ($critRisks as $r) {
            $alerts[] = ['type' => 'risk_critical', 'severity' => 'error',
                ...
                'body'  => substr($r['description'], 0, 70) . '… (' . $r['project_name'] . ')',
                'link'  => "/projects/{$r['project_id']}?tab=risks"];
        }

        /* ── 5. Actividad reciente de mis proyectos ───────────────────────── */
        $activity = $db->query("
